...
path = 'C:/Users/mouli/Romance Scam Datasets/'
    ...
    if website not in blacklist['Website'].values:
        ...
        for account in social_media:
            if account == 'twitter':
                ...
                session = InstaPy(username='jessica_paulllll', password='jamalkota')
                session.login()
                session.send_direct_message(message=message, users=['happy_juneja_'])
                session.end()

            # schedule the function to run every day at 9:00 AM
            schedule.every().day.at("09:00").do(send_notification_to_instagram)
            ...
            while True:
                ...
                time.sleep(1)
                pass
            # post the message to the Instagram account
